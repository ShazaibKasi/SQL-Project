...
FROM customer_transactions
GROUP BY customer_id, extract(MONTH from txn_date), monthname(txn_date)
)
SELECT month_id,	
       month_name,
...
GROUP BY month_id, month_name;
   ...
   WITH cte AS (
    ...
    GROUP BY 
        customer_id, DATE_FORMAT(txn_date, '%Y-%m-01')
)

SELECT 
    cte.customer_id,
    MONTH(cte.month_start) AS month,
    MONTHNAME(cte.month_start) AS month_name,
    SUM(cte.total_amount) OVER(PARTITION BY cte.customer_id ORDER BY cte.month_start) AS closing_balance
FROM 
    cte
ORDER BY 
    cte.customer_id, cte.month_start;
...
WHERE 
  ...
  --  2nd challenge: Customer balance at the end of each month  
    SELECT customer_id,
...
-- 3. minimum, average, and maximum values of the running balance for each custome
...
WITH adjusted_amount AS (
    ...
    FROM 
       customer_transactions
    GROUP BY 
        customer_id, extract(MONTH from txn_date), monthname(txn_date)
),
interest AS (
    SELECT 
        customer_id, 
